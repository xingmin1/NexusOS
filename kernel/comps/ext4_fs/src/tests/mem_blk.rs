//! ext4_fs — 内核端综合测试集
//!
//! 运行方式：`cargo ktest --package ext4_fs`
//!
//! 组织：
...
use vfs::{
    AsyncBlockDevice, AsyncFileSystemProvider, DirectoryEntry, OpenFlags, VfsManager,
...
impl TestCtx {
    fn new() -> Self {
        ...
        let provider = get_ext4_provider() as Arc<dyn AsyncFileSystemProvider + Send + Sync>;
        ...
        let root = block_on(async { vfs.root().await }).expect("root vnode");
...
fn test_mount_statfs() {
    let t = TestCtx::new();
    let stats = (async { t.vfs.statfs("/") }).block().unwrap();
    info!(?stats, "statfs");
...
fn test_lookup_read() {
    let t = TestCtx::new();
    let etc = (async { t.root.lookup("etc".as_ref()) }).block().unwrap();
    assert_eq!(etc.kind(), VnodeType::Directory);
    // 读取一个已知文件
    let f = (async { etc.lookup("passwd".as_ref()) }).block().unwrap();
    let h = (async { f.open_file_handle(OpenFlags::RDONLY) }).block().unwrap();
    let mut buf = [0u8; 16];
    let n = (async { h.read_at(0, &mut buf) }).block().unwrap();
...
fn test_create_write_read() {
    ...
    let n = (async { h.read_at(0, &mut readback) }).block().unwrap();
...
fn test_mkdir_readdir_rmdir() {
    ...
    dir.mkdir("b".as_ref(), FileMode::OWNER_RWE).block().unwrap();
    let open_flag = OpenFlags::RDONLY;
    let dh = dir.open_dir_handle(open_flag).block().unwrap();
    let mut names = Vec::<String>::new();
    let mut d = dh.clone();
    while let Some(e) = d.readdir().block().unwrap() {
        names.push(e.name);
    }
...
fn test_rename_unlink() {
    let t = TestCtx::new();
    let tmp = t.root.mkdir("rename".as_ref(), FileMode::OWNER_RWE).block().unwrap();
    let f = tmp.create_node("old".as_ref(), VnodeType::File, FileMode::OWNER_RW, None).block().unwrap();
    tmp.rename("old".as_ref(), None, "new".as_ref()).block().unwrap();
    tmp.unlink("new".as_ref()).block().unwrap();
}

#[ktest(ignore)]
fn test_parallel_rw() {
    use ostd::task::{yield_now, scheduler::spawn};
    let t = TestCtx::new();
    let tmp = (async { t.root.mkdir("p".as_ref(), FileMode::OWNER_RWE) }).block().unwrap();
    ...
    let writer = |idx: u32| {
        ...
        async move {
            ...
            let v = tmp.clone().create_node(name.as_ref(), VnodeType::File, FileMode::OWNER_RW, None)
    ...
    let core = ostd::task::scheduler::Core::new();
