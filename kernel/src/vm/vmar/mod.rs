...
use ostd::{
    ...
    mm::{
        tlb::TlbFlushOp, PageFlags, PageProperty, Vaddr, VmSpace, MAX_USERSPACE_VADDR, PAGE_SIZE,
    },
    sync::RwLock,
...
//     default async fn handle_page_fault(&self, _page_fault_info: &PageFaultInfo) -> Result<()> {
//         unimplemented!()
//     }
// }
...
impl<R> Vmar<R> {
    ...
    pub async fn resize_mapping(
        ...
        self.0.resize_mapping(map_addr, old_size, new_size).await
    }
}
...
impl Vmar_ {
    ...
    fn new_root() -> Arc<Self> {
        ...
        vm_space.register_page_fault_handler(handle_page_fault_sync_wrapper);
        Vmar_::new(vmar_inner, Arc::new(vm_space), 0, ROOT_VMAR_CAP_ADDR)
    }

    async fn protect(&self, perms: VmPerms, range: Range<usize>) -> Result<()> {
        assert!(range.start % PAGE_SIZE == 0);
        assert!(range.end % PAGE_SIZE == 0);
        self.do_protect_inner(perms, range).await
    }

    // Do real protect. The protected range is ensured to be mapped.
    async fn do_protect_inner(&self, perms: VmPerms, range: Range<usize>) -> Result<()> {
    ...
    pub async fn handle_page_fault(&self, page_fault_info: &PageFaultInfo) -> Result<()> {
        ...
        if let Some(vm_mapping) = inner.vm_mappings.find_one(&address) {
            ...
            return vm_mapping
                .handle_page_fault(&self.vm_space, page_fault_info)
                .await;
        }
    ...
    async fn clear_root_vmar(&self) -> Result<()> {
    ...
    async fn resize_mapping(
        ...
        if new_size < old_size {
            self.remove_mapping(new_map_end..old_map_end).await?;
            return Ok(());
        }
    ...
    pub(super) async fn new_fork_root(self: &Arc<Self>) -> Result<Arc<Self>> {
        let new_vmar_ = {
            let vmar_inner = VmarInner::new();
            let mut new_space = VmSpace::new();
            new_space.register_page_fault_handler(handle_page_fault_sync_wrapper);
            Vmar_::new(vmar_inner, Arc::new(new_space), self.base, self.size)
        };

        {
            let inner = self.inner.read().await;
            let mut new_inner = new_vmar_.inner.write().await;
...
fn handle_page_fault_sync_wrapper(
    vm_space: &VmSpace,
    trap_info: &CpuExceptionInfo,
) -> core::result::Result<(), ()> {
    handle_page_fault_from_vm_space(vm_space, &trap_info.try_into().unwrap()).block()
}
...
impl<R1, R2> VmarMapOptions<R1, R2> {
    ...
    pub async fn build(self) -> Result<Vaddr> {
        self.check_options()?;
        let Self {
            parent,
            vmo,
        ...
        let mut inner = parent.0.inner.write().await;
