...
impl VmMapping {
    pub async fn handle_page_fault(
        ...
        if !is_write && self.vmo.is_some() && self.handle_page_faults_around {
            self.handle_page_faults_around(vm_space, address).await?;
            return Ok(());
        }
...
impl MappedVmo {
    ...
    async fn get_committed_frame(&self, page_offset: usize) -> Result<UFrame> {
        debug_assert!(page_offset < self.range.len());
        debug_assert!(page_offset % PAGE_SIZE == 0);
        self.vmo.commit_page(self.range.start + page_offset).await
    }
    ...
    async fn operate_on_range<F>(&self, range: &Range<usize>, operate: F) -> Result<()>
    where
        F: FnMut(&mut dyn FnMut() -> Result<UFrame>) -> Result<()>,
    {
        ...
        self.vmo.operate_on_range(&range, operate).await
    }
