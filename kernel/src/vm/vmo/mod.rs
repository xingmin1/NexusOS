...
impl Clone for Pages {
    fn clone(&self) -> Self {
        match self {
            Self::Nonresizable(_, _) => self
                .with(|pages, size| Self::Nonresizable(Mutex::new(pages.clone()), size))
                .block(),
            Self::Resizable(_) => self
                .with(|pages, size| Self::Resizable(Mutex::new((pages.clone(), size))))
                .block(),
        }
    }
}
...
impl Pages {
    async fn with<R, F>(&self, func: F) -> R
    where
        F: FnOnce(&mut XArray<UFrame>, usize) -> R,
    {
        match self {
            Self::Nonresizable(pages, size) => func(pages.lock().await.deref_mut(), *size),
            Self::Resizable(pages) => {
                let mut lock = pages.lock().await;
                let size = lock.1;
...
impl Vmo_ {
    ...
    async fn decommit_page(&mut self, offset: usize) -> Result<()> {
        let page_idx = offset / PAGE_SIZE;
        self.pages
            .with(|pages, size| {
                if offset >= size {
    ...
    pub async fn operate_on_range<F>(
        &self,
        range: &Range<usize>,
        mut operate: F,
        commit_flags: CommitFlags,
    ...
    pub async fn read(&self, offset: usize, writer: &mut VmWriter<'_>) -> Result<()> {
        let read_len = writer.avail().min(self.size().await.saturating_sub(offset));
    ...
    pub async fn write(&self, offset: usize, reader: &mut VmReader<'_>) -> Result<()> {
    ...
    pub async fn clear(&self, range: Range<usize>) -> Result<()> {
        ...
        self.write(range.start, &mut reader).await?;
        Ok(())
    }

    /// Returns the size of current VMO.
    pub async fn size(&self) -> usize {
        self.pages.with(|_, size| size).await
    }
    ...
    pub async fn is_page_committed(&self, page_idx: usize) -> bool {
        self.pages
    ...
    async fn replace(&self, page: UFrame, page_idx: usize) -> Result<()> {
        self.pages
            .with(|pages, size| {
                if page_idx >= size / PAGE_SIZE {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "the page index is outside of the vmo"
                    );
                }
