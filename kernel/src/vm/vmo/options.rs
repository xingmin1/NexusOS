...
use crate::{error::Result, vm::vmo::Vmo_};
...
mod test {
    ...
    use ostd::{mm::VmIo, prelude::*, task::scheduler::blocking_future::BlockingFuture};
    ...
    fn alloc_vmo() {
        ...
        assert_eq!(vmo.size().block(), PAGE_SIZE);
    ...
    fn alloc_continuous_vmo() {
        let vmo = VmoOptions::<Full>::new(10 * PAGE_SIZE)
            ...
            .alloc()
            .unwrap();
        assert_eq!(vmo.size().block(), 10 * PAGE_SIZE);
    }
    ...
    fn resize() {
        ...
        vmo.resize(2 * PAGE_SIZE).block().unwrap();
        assert_eq!(vmo.size().block(), 2 * PAGE_SIZE);
        assert_eq!(vmo.read_val::<u8>(10).unwrap(), 42);
        vmo.write_val(PAGE_SIZE + 20, &123u8).unwrap();
        vmo.resize(PAGE_SIZE).block().unwrap();
        assert_eq!(vmo.read_val::<u8>(10).unwrap(), 42);
    }
}
