...
impl<G: PinCurrentCpu> TlbFlusher<G> {
    ...
    pub fn issue_tlb_flush_with(&self, op: TlbFlushOp, drop_after_flush: Frame<Unknown>) {
    ...
    fn issue_tlb_flush_(&self, op: TlbFlushOp, drop_after_flush: Option<Frame<Unknown>>) {
...
cpu_local! {
    ...
    static PAGE_KEEPER: SpinLock<Vec<Frame<Unknown>>, LocalIrqDisabled> = SpinLock::new(Vec::new());
}
